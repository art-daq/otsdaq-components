...
class FECommanderInterface : public FEVInterface, public TransceiverSocket
{
  public:
	...
	{
		__FE_SS__ << "Universal read not defined (addressPtr=" << (address ? '1' : '0')
		          << "returnPtr=" << (returnValue ? '1' : '0') << __E__;
		__FE_SS_THROW__;
	}
	void universalWrite(char* address, char* writeValue) override
	{
		__FE_SS__ << "Universal write not defined (addressPtr=" << (address ? '1' : '0')
		          << "writePtr=" << (writeValue ? '1' : '0') << __E__;
		__FE_SS_THROW__;
	}
